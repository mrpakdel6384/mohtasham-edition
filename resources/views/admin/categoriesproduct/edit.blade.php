    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="card">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" action="{{ route('admin.categoriesproduct.update' , $categoriesproduct->id) }}" method="POST">
                    @csrf
                    @method('patch')
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control" id="inputEmail3" placeholder="نام دسته را وارد کنید" value="{{ old('name' , $categoriesproduct->name) }}">
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" name="parent_id" id="permissions">
                                ...
                                @foreach(\App\CategoryProduct::all()->except($categoriesproduct->id) as $cate)
                                    <option value="{{ $cate->id }}" {{ $cate->id === $categoriesproduct->parent_id ? 'selected' : '' }}>{{ $cate->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
