            ...
            <div class="row">
                ...
                <div class="col-lg-9 content py-4 mb-2 mb-sm-0 pb-sm-5 d-rtl">
                    ...
                    <article class="card card-horizontal card-hover mb-grid-gutter">
                        <a class="card-img-top @if($loop->iteration % 2 !==0) order-sm-2 @endif" href="{{route('content.single',['category'=>$category->slug,'content'=>$content->slug])}}" style="background-image: url({{$content->thumbImage(300)}});"></a>
                        <div class="card-body @if($loop->iteration % 2 !==0) order-sm-1 @endif">
                            <a class="meta-link fs-sm mb-2" href="{{route('content.blog',['category'=>$category_sidebar->slug])}}">{{$category->title}}</a>
                            <h2 class="h4 nav-heading mb-4">
                                <a href="{{route('content.single',['category'=>$category->slug,'content'=>$content->slug])}}">{{$content->title}}</a>
                            </h2>
                            @php
